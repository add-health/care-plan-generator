...
// Drive calls have no timeout of their own. If Drive is slow or unreachable the
// handler hangs forever, which means the response never comes and the temp file
// is never cleaned up. That was survivable when a human pressed Save; now that
// every generated plan files itself it would leak a file per plan.
const DRIVE_TIMEOUT_MS = 30000;
function withTimeout(promise, label, ms = DRIVE_TIMEOUT_MS) {
  let timer;
  return Promise.race([
...
app.post('/save-pdf', async (req, res) => {
  ...
  try {
    ...
    const uploaded = await withTimeout(drive.files.create({
...
// ── Caregiver PDF → Google Drive ────────────────────────────────────
// Mirrors /save-pdf. Deliberately a separate handler rather than a refactor of
// that one: it is the live physiotherapy path and is not worth disturbing.
app.post('/save-caregiver-pdf', async (req, res) => {
  const { patient, plan } = req.body;
  ...
  try {
    ...
    console.error('Caregiver PDF save error:', err);
    res.status(500).json({ error: err.message });
  } finally {
    if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath);
  }
